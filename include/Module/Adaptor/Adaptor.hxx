...
Adaptor::Adaptor(const size_t n_elmts, const std::type_index datatype, const size_t buffer_size)
  ...
  , buffer(new std::vector<std::vector<std::vector<int8_t*>>>(
      1,
      std::vector<std::vector<int8_t*>>(1, std::vector<int8_t*>(buffer_size))))
...
size_t
Adaptor::get_n_elmts(const size_t sid) const
{
    return this->n_elmts[sid];
}

size_t
Adaptor::get_n_bytes(const size_t sid) const
{
    return this->n_bytes[sid];
}

std::type_index
Adaptor::get_datatype(const size_t sid) const
{
    return this->datatype[sid];
}

bool
Adaptor::is_empty(const size_t id)
{
    return (*this->first)[id] == (*this->last)[id];
}

bool
Adaptor::is_full(const size_t id)
{
...
size_t
Adaptor::n_fill_slots(const size_t id)
{
    return (*this->last)[id] - (*this->first)[id];
}

size_t
Adaptor::n_free_slots(const size_t id)
{
