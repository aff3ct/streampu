...
main(int argc, char** argv)
{
    ...
    runtime::Pipeline pipeline_chain =
      spu::tools::Pipeline_builder()
        .add_task_for_checking(source("generate"))
        .add_stage(spu::tools::Pipeline_builder::Stage_builder()
                     .add_first_task(source("generate"))
                     .add_last_task(source("generate"))
                     .set_n_threads(1))
        .configure_interstage_synchro(spu::tools::Pipeline_builder::Synchro_builder()
                                        .set_active_waiting(active_waiting)
                                        .set_buffer_size(buffer_size))
        .add_stage(spu::tools::Pipeline_builder::Stage_builder()
                     .add_first_task((*rlys[0])("relay"))
                     .add_last_task((*rlys[rlys.size() - 1])("relay"))
                     .set_n_threads(n_threads ? n_threads : 1))
        .configure_interstage_synchro(spu::tools::Pipeline_builder::Synchro_builder()
                                        .set_active_waiting(active_waiting)
                                        .set_buffer_size(buffer_size))
        .add_stage(spu::tools::Pipeline_builder::Stage_builder().add_first_task(sink("send_count")).set_n_threads(1))
        .build();
