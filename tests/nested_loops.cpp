...
int main(int argc, char** argv)
{
	...
	unsigned int limit = n_exec ? (n_exec * n_threads) / (iterator.get_limit() * iterator2.get_limit()) : 1;
	...
	auto theoretical_time = ((chain_sleep_time * limit * n_inter_frames) / 1000.f / 1000.f / n_threads) * (iterator.get_limit() * iterator2.get_limit());
