...
int main(int argc, char** argv)
{
	...
	std::cout << "####################################################" << std::endl;
	std::cout << "# Micro-benchmark: Simple pipeline multiple stages #" << std::endl;
	std::cout << "####################################################" << std::endl;
	std::cout << "#" << std::endl;
    ...
    std::vector<std::shared_ptr<module::Relayer_fwd<uint8_t>>> rlys_fwd(20);
	...
	// sockets binding
	(*rlys_fwd[0])[module::rly_fwd::sck::relay_fwd::fwd] = source[module::src::sck::generate::out_data]; // Bind du premier relay_fwd avec la source
    ...
    sink[module::snk::sck::send_count::in_data] = (*rlys_fwd[rlys_fwd.size() -1])[module::rly_fwd::sck::relay_fwd::fwd]; // Bind du dernier IO avec le premier élement de l'étage suivant
	...
	else 
	{
		pipeline_chain.reset(new runtime::Pipeline(
		                     ...
		                     { // pipeline stage 0
		                       ...
		                       // pipeline stage 1
		                       { { &(*rlys_fwd[             0])[module::rly_fwd::tsk::relay_fwd] },   // first tasks of stage 1
		                         { &(*rlys_fwd[1])[module::rly_fwd::tsk::relay_fwd]}}, // last  tasks of stage 1
		                       // pipeline stage 2
		                       { {&(*rlys_fwd[2])[module::rly_fwd::tsk::relay_fwd]},   // first tasks of stage 2
		                         {&(*rlys_fwd[10])[module::rly_fwd::tsk::relay_fwd]}}, // last  tasks of stage 2
                               // pipeline stage 3
                               { {&(*rlys_fwd[11])[module::rly_fwd::tsk::relay_fwd]},   // first tasks of stage 3
		                         {&(*rlys_fwd[11])[module::rly_fwd::tsk::relay_fwd]}},	  // last  tasks of stage 3
                               // pipeline stage 4
                               { {&(*rlys_fwd[12])[module::rly_fwd::tsk::relay_fwd]},   // first tasks of stage 4
		                         {&(*rlys_fwd[rlys_fwd.size()-1])[module::rly_fwd::tsk::relay_fwd]} }, // last tasks of stage 4
                               // pipeline stage 5
		                     ...
		                     },
		                     {
                               ...
                               1,                         // number of threads in the stage 5    
		                     },
		                     {
		                     ...
		                     },
		                     {
                               ...
                               active_waiting, // type of waiting between stages 4 and 5 (true = active, false = passive)
		                     }));
	...
	auto theoretical_time = (n_frames * ((rlys_fwd.size()) * sleep_time_us * 1000) * n_inter_frames) / 1000.f / 1000.f / n_threads;
	...
	(*rlys_fwd[0])[module::rly_fwd::sck::relay_fwd::fwd].unbind(source[module::src::sck::generate::out_data]); // Unbind de la source et la première tache FWD


	for (size_t s = 0; s < rlys_fwd.size() -1; s++)
		(*rlys_fwd[s+1])[module::rly_fwd::sck::relay_fwd::fwd].unbind((*rlys_fwd[s])[module::rly_fwd::sck::relay_fwd::fwd]);
    
    sink[module::snk::sck::send_count::in_data].unbind((*rlys_fwd[rlys_fwd.size() -1])[module::rly_fwd::sck::relay_fwd::fwd]);
