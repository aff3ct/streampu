...
main(int argc, char** argv)
{
    ...
    if (sck_type_tsk[0][0] == "SFWD")
    {
        if (task_type == "relayer")
            (*(module::Relayer<uint8_t>*)(tasks[tas].get()))[module::rly::sck::relayf::fwd].unbind(
              (*((module::Source_user_binary<uint8_t>*)(first_task)))[module::src::sck::generate::out_data]);
        else
            (*(module::Incrementer<uint8_t>*)(tasks[tas].get()))[module::inc::sck::incrementf::fwd].unbind(
              (*((module::Initializer<uint8_t>*)(first_task)))[module::ini::sck::initialize::out]);
    }
    else
    {
        if (task_type == "relayer")
            (*(module::Relayer<uint8_t>*)(tasks[tas].get()))[module::rly::sck::relay::in].unbind(
              (*((module::Source_user_binary<uint8_t>*)(first_task)))[module::src::sck::generate::out_data]);
        else
            (*(module::Incrementer<uint8_t>*)(tasks[tas].get()))[module::inc::sck::increment::in].unbind(
              (*((module::Initializer<uint8_t>*)(first_task)))[module::ini::sck::initialize::out]);
    }
    for (size_t i = 0; i < sck_type_tsk.size(); ++i)
    {
            ...
            if (sck_type_tsk[i][j + 1] == "SFWD")
            {
                if (sck_type_tsk[i][j] == "SFWD")
                {
                    if (task_type == "relayer")
                        ...
                        (*(module::Incrementer<uint8_t>*)(tasks[tas + 1].get()))[module::inc::sck::incrementf::fwd]
                          .unbind(
            ...
            else
            {
                if (sck_type_tsk[i][j] == "SFWD")
                {
                    if (task_type == "relayer")
                        ...
                        (*(module::Incrementer<uint8_t>*)(tasks[tas + 1].get()))[module::inc::sck::increment::in]
                          .unbind(
                            (*(module::Incrementer<uint8_t>*)(tasks[tas].get()))[module::inc::sck::incrementf::fwd]);
                }
                else
    ...
    {
        if (task_type == "relay")
            (*((module::Sink_user_binary<uint8_t>*)(last_task)))[module::snk::sck::send_count::in_data].unbind(
              ((*(module::Relayer<uint8_t>*)(tasks[tas].get())))[module::rly::sck::relayf::fwd]);
        else
            (*((module::Finalizer<uint8_t>*)(last_task)))[module::fin::sck::finalize::in].unbind(
              ((*(module::Incrementer<uint8_t>*)(tasks[tas].get())))[module::inc::sck::incrementf::fwd]);
    }
    else
    {
    ...
    if (task_type == "relay")
        (*((module::Sink_user_binary<uint8_t>*)(last_task)))[module::snk::sck::send_count::in_count].unbind(
          (*((module::Source_user_binary<uint8_t>*)(first_task)))[module::src::sck::generate::out_count]);
    return test_results;
}