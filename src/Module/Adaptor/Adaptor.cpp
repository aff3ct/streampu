...
void
Adaptor::deep_copy(const Adaptor& m)
{
    Module::deep_copy(m);

    int id = -1;
    for (size_t i = 0; i < this->buffer->size(); i++)
        if ((*this->buffer)[i].size() == 0) id = (int)i;
...
void
Adaptor::set_no_copy_pull(const bool no_copy_pull)
{
    this->no_copy_pull = no_copy_pull;
}

void
Adaptor::set_no_copy_push(const bool no_copy_push)
{
    this->no_copy_push = no_copy_push;
}

bool
Adaptor::is_no_copy_pull()
{
    return this->no_copy_pull;
}

bool
Adaptor::is_no_copy_push()
{
    return this->no_copy_push;
}

void
Adaptor::reset_buffer()
{
    size_t id_buff = 0;
    for (size_t s = 0; s < this->n_sockets; s++)
        for (size_t b = 0; b < this->buffer_size; b++)
...
void
Adaptor::set_n_frames(const size_t n_frames)
{
